...
| B34 | The LOOK and WEB pads sit on top of the bottom of the two sticks in portrait | Claude 2026-09-02, found during B28 | proposed | Measured at 390x844 with an iPhone 14 Pro's insets: the LOOK pad is 80x31 px and sits entirely inside the left stick's 136 px ring, WEB the same over the right. Both are z-index 40 against the HUD's 10, and elementFromPoint inside the overlap returns the pad, so a thumb sliding down the bottom of a stick hands its touch to LOOK. Pre-existing, not caused by v0.3.0. Left alone because B28 was scoped to landscape and the portrait layout was not to be redesigned. The fix is a layout decision (raise the pads, shrink the sticks, or move the pads inboard), which is why it wants its own row. Maor plays on a phone in portrait, so this is the most likely thing to bite him. |
